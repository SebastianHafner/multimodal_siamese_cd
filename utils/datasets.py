...
class StockholmCDDataset(AbstractMultimodalCDDataset):

    def __init__(self, cfg: experiment_manager.CfgNode, run_type: str, sthlm_data_path: str,
                 no_augmentations: bool = False, dataset_mode: str = None, disable_multiplier: bool = False,
                 disable_unlabeled: bool = False):
        super().__init__(cfg, run_type)

        self.sthlm_data_path = Path(sthlm_data_path)
        self.sthlm_dates = list(cfg.DATALOADER.STOCKHOLM_DATES)
        self.dataset_mode = cfg.DATALOADER.DATASET_MODE if dataset_mode is None else dataset_mode
        self.include_building_labels = cfg.DATALOADER.INCLUDE_BUILDING_LABELS
        ...
        # include unlabeled data from stockholm
        if cfg.DATALOADER.INCLUDE_UNLABELED and not disable_unlabeled:
            assert (run_type == 'train')
            assert(cfg.AUGMENTATION.CROP_SIZE <= 256)
            self.tiles_stockholm = geofiles.load_json(self.sthlm_data_path / f'tiles_stockholm.json')
            self.aoi_ids.extend(self.tiles_stockholm)
            self.labeled.extend([False] * len(self.tiles_stockholm))
    ...
    def __getitem__(self, index):
        ...
        if 'stockholm' in aoi_id:

            patch_id = aoi_id.split('_')[-1]
            indices = sorted(np.random.randint(0, len(self.sthlm_dates), size=2))
            # t1
            date_t1 = self.sthlm_dates[indices[0]]
            img_s1_t1 = self._load_s1_img_sthlm(date_t1, patch_id)
            img_s2_t1 = self._load_s2_img_sthlm(date_t1, patch_id)

            # t2
            date_t2 = self.sthlm_dates[indices[-1]]
            img_s1_t2 = self._load_s1_img_sthlm(date_t2, patch_id)
            img_s2_t2 = self._load_s2_img_sthlm(date_t2, patch_id)
    ...
    def _load_s1_img_sthlm(self, date: str, patch_id: str) -> np.ndarray:
        file = self.sthlm_data_path / f's1_stockholm_{date}' / f's1_stockholm_{date}-{patch_id}.tif'
        img, _, _ = geofiles.read_tif(file)
        img = np.clip(img[:, :, self.s1_band_indices], 0, 1)
        return np.nan_to_num(img).astype(np.float32)

    def _load_s2_img_sthlm(self, date: str, patch_id: str) -> np.ndarray:
        file = self.sthlm_data_path / f's2_stockholm_{date}' / f's2_stockholm_{date}-{patch_id}.tif'
        img, _, _ = geofiles.read_tif(file)
        img = np.clip(img[:, :, self.s2_band_indices], 0, 1)
        return np.nan_to_num(img).astype(np.float32)