...
def run_training(cfg):
    ...
    # reset the generators
    dataset = datasets.MultimodalCDDataset(cfg=cfg, run_type='train', balanced=True)
    print(dataset)
    ...
    for epoch in range(warmup_epochs + 1, epochs + 1):
        ...
        for i, batch in enumerate(dataloader):
            ...
            if not is_labeled.all():
                ...
                y_hat_change_noisy = torch.sigmoid(logits_change_noisy)
                if cfg.CONSISTENCY_TRAINER.LOSS_TYPE == 'L2':
                    y_hat_change = torch.sigmoid(logits_change)
                    cons_loss = cons_criterion(y_hat_change[is_not_labeled], y_hat_change_noisy[is_not_labeled])
                else:
                    cons_loss = cons_criterion(logits_change[is_not_labeled], y_hat_change_noisy[is_not_labeled])
